...
class ProgressiveTrainer:
    ...
    def step_discriminator(self, dim_ind, phase, iteration, global_iteration):
        ...
        seg_mc = torch.FloatTensor(self.progressive_model_on_one_gpu.batch_size[dim_ind], self.num_semantics, seg.size(2), seg.size(3)).zero_().cpu()
...
if __name__ == "__main__":
    ...
    trainer = ProgressiveTrainer(opt)

        
    fixed_z = torch.randn(16, 512)
    dim_ind = int(np.log2(trainer.progressive_model_on_one_gpu.dim))-2
    trainer.num_semantics = trainer.progressive_model_on_one_gpu.num_semantics
    global_iteration = trainer.progressive_model_on_one_gpu.global_iteration
    res_init = trainer.progressive_model_on_one_gpu.generator.res
    trainer.set_data_resolution(256)

    seg, _ = trainer.next_batch()
    print(seg.size())
    print(seg[0])
    print(seg[0].long())
    ...
    ax.imshow(make_grid((seg[0,:,:,:]), nrow=1).permute(1, 2, 0).long())
    plt.show()
        
    seg = seg.cpu()
    seg_mc = torch.FloatTensor(36, trainer.num_semantics, seg.size(2), seg.size(3)).zero_().cpu()
    
    print()
    print("seg_mc size = ",seg_mc.size())
