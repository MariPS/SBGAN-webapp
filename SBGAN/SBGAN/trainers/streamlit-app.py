...
class ProgressiveTrainer:
    ...
    def call_next_batch(self, seg, im):
        ...
        seg = torch.Tensor(seg).cpu()
        seg_mc = torch.FloatTensor(self.opt.batchSize, self.num_semantics, seg.size(2), seg.size(3)).zero_().cpu()
    ...
    def run(self):
        ...
        z_fid = torch.randn(self.opt.nums_fid, 512).cpu()
        epoch_start = 0
    ...
    def test_model(self , global_iteration, n_sample=1000, input_img=None):
        ...
        z_predefined = torch.Tensor(np.load('z_fixed.npy')).cpu()
        num_bs = 1
    ...
    def compute_FID(self, global_iteration, real_fake='fake', z_fixed=torch.Tensor([])):
        ...
        for i in range(int(nums_fid/batchsize)):
            ...
            real_ims = Variable(real_ims).cpu()
            real_segs = real_segs.cpu()
            ...
            if z_fixed.nelement() == 0:
                z = torch.randn(batchsize, 512).cpu()
            else:
            ...
            if real_fake=='fake':
                ...
                real_segs_mc = torch.FloatTensor(real_segs.size(0), fake.size(1), real_segs.size(2), real_segs.size(3)).zero_().cpu()
...
def load_model_weights():
    ...
    progressive_segG_weights_PATH = "weights/ft_ade_indoor_end2end/376_net_G.pth"
    progressive_segD_weights_PATH = "weights/ft_ade_indoor_end2end/376_net_D.pth"
    ...
    return progressive_weights_PATH
...
if __name__ == "__main__":
    ...
    # opt.label_nc = opt.label_seg
    # opt.test = True
    # opt.name = "ft_ade_indoor_end2end"
    # opt.dataset = "ade_indoor"
    # opt.dataset_mode = "ade_indoor"
    # opt.dataroot = "datasets/ade_indoor"
    # opt.num_semantics = 95
    # opt.label_seg = 95
    # opt.nums_fid = 433
    # opt.which_iter_D2 = 747370
    # opt.which_iter = 742993
    # opt.which_epoch = 351
    # opt.load_size = 256
    # opt.crop_size = 256
    ...
    if opt.test:
        ...
        st.title("SBGAN on ADEindoor")
        st.write("The model takes as input an image segmentation (.png) and outputs a new image with textures for specified semantic segemntation. It specializes in indoor environments' furnitures like chairs, tables, etc.")

        input_option = st.selectbox(
            'Type of input',
            ('Upload image', 'Draw segmentation online'))

        if input_option == 'Upload image':
            image_file = st.file_uploader("Upload", type=["png"])
            ...
            if image_file is not None:
                ...
                with open("datasets/ADE_indoor/ADE_train_00000192.png","wb") as f: 
                ...
                st.write("Loading...")
                ProgressiveTrainer(opt).test_model(opt.which_iter, n_sample=opt.N, input_img=img)

                new_img = Image.open("samples/ft_ade_indoor_end2end/0_spade_ffr_256_852418.png")
                #new_img = new_img.transpose(Image.FLIP_LEFT_RIGHT)
                st.image(new_img,width=250)
                new_img.close()
                img.close()

        if input_option == 'Draw segmentation online':
            ...
            item_type = st.sidebar.selectbox(
                "Item type:", ("wall", "floor", "ceiling", "door", "table", "chair", "couch", "coffee table")
            )
            ...
            if canvas_result.image_data is not None:
                ...
                if st.button("Predict"):
                    ...
                    canvas_img = canvas_img[:, :, :-1] # Ignore alpha channel
                    print(canvas_img.shape)
                    canvas_img = Image.fromarray(canvas_img)
                    canvas_img.save("datasets/ADE_indoor/ADE_train_00000192.png", "png")
                    #cv2.imwrite(f"datasets/ADE_indoor/ADE_train_00000192.png",  canvas_result.image_data)
                    ...
                    st.write("Loading...")
                    ProgressiveTrainer(opt).test_model(opt.which_iter, n_sample=opt.N, input_img=canvas_img)
