...
class SupDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #X_lbl = X_lbl * 255.0
        X_lbl = transforms.functional.rgb_to_grayscale(X_lbl)
        # squeeze pixel values into range [0,num_semantics]
        X_lbl = X_lbl * self.opt.num_semantics
    ...
    def __getitem__(self, index):
        ...
        if 'lbl' in self.dataset_name:
            ...
            if self.transform is not None:
                X = self.transform(X)
            if self.dataset_name == 'ade_indoor_lbl': #BEFORE: ADE_indoor_lbl
                # X = X
                X = transforms.functional.rgb_to_grayscale(X)
                # squeeze pixel values into range [0,num_semantics]
                X = X * self.opt.num_semantics
            else:
                X = X * 255.0
