...
class ProgressiveSegModel(torch.nn.Module):
    ...
    def initialize_networks(self, D_inputs=0):
        ...
        generator = ProGANGenerator(max_dim=self.opt.max_dim, rgb=self.opt.rgb, num_semantics=self.opt.num_semantics, T=self.opt.T, aspect_ratio=aspect_ratio).cpu()
        discriminator = ProGANDiscriminator(max_dim=self.opt.max_dim, rgb=self.opt.rgb, num_semantics=D_inputs, aspect_ratio=aspect_ratio).cpu()
    ...
    def compute_generator_loss(self, iteration, global_iteration, dim_ind,
        ...
        if z.nelement() == 0:
            z = torch.randn(self.batch_size[dim_ind], 512).cpu()
        if interpolate:
    ...
    def gumbelSampler(self, fake, hard=True, eps=1e-10, dim=1):
        ...
        if hard:
            ...
            return index.type(torch.FloatTensor), ret
        else:
    ...
    def inferenceSampler(self,fake, scaling, num_semantics):
        ...
        x_fake = index.type(torch.FloatTensor)
    ...
    def load_model(self, global_iter, load_all=True):
        # load params
        checkpoint = torch.load('%s/weights/%s/%s.pth'%(self.opt.save_path, self.opt.name, global_iter),map_location=torch.device('cpu'))
        res = checkpoint['dim']
