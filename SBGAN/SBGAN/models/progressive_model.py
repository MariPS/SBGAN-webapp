...
class ProgressiveModel(torch.nn.Module):
    ...
    def initialize_networks(self):
        if 'cityscapes' in self.opt.dataset:
            ...
            aspect_ratio = 1
        generator = ProGANGenerator(max_dim=self.opt.max_dim, rgb=self.opt.rgb, aspect_ratio=aspect_ratio).cpu()
        discriminator = ProGANDiscriminator(max_dim=self.opt.max_dim, rgb=self.opt.rgb, aspect_ratio=aspect_ratio).cpu()
